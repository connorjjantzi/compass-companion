...
export default async function fetchItems(
  ...
  while (retries < maxRetries) {
    ...
    Object.entries(counts).forEach(([name, quantity]) => {
      ...
      if (
        compassPrice.userValue &&
        compassPrice.userValue > compassPrice.chaos
      ) {
        compassList.push({
          name,
          quantity,
          value: compassPrice.chaos,
          totalValue: compassPrice.userValue * quantity,
          userValue: compassPrice.userValue,
        });
      } else {
        compassList.push({
          name,
  ...
  console.log(
    `Failed to fetch stash items for ${stashTab.name} after ${maxRetries} retries`
  );
  return [];
}
